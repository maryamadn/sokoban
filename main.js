...
const app = {
  ...
  currentMap: [],
  maps: [
    //put in separate js file and import/export eg export function setupCounter(element) and  imprt {setupCounter} from 'file.js'
    `####
# .#
#  ###
#*@  #
...
const originalMapProcessing = () => {
  //to render map readable (from string to array)
...
const checkForWin = () => {
  //check if there are any boxes not on goal ($)
  ...
  if (combinedMapArray.every((x) => x !== "$")) {
    ...
    const $winBannerMain = $("<p>")
      .addClass("winBannerMain")
      .text("LEVEL COMPLETE!");
    const $winBannerSub = $("<p>")
      .addClass("winBannerSub")
...
const paintMap = () => {
  ...
  const $div = $("<div>").addClass("updown");
  $(".arrows").append($div);

  const $up = $("<i>").addClass("fa-solid fa-square-caret-up fa-3x up");
  $(".updown").append($up);
  ...
  const $right = $("<i>").addClass(
    "fa-solid fa-square-caret-right fa-3x right"
  );
  $(".arrows").append($right);
...
const resetAndQuit = () => {
  ...
  $(".quitButton").on("click", () => {
    //hide this and go to username page
    $(".gamePage").removeClass("shown");
    $(".mainPage").addClass("shown");
    $(".hidden").hide();
    $(".shown").show();
...
const playerOriginalPosition = () => {
  for (const row in app.currentMap) {
    const position = [];
    for (const column in app.currentMap[row]) {
      //why is this no different froma just app.currentMap
      if (
        app.currentMap[row][column] === "@" ||
        app.currentMap[row][column] === "+"
      ) {
        //check if the element is player or player on goal
...
$(".leaderboardButton").on("click", () => {
  ...
  if (localStorage.length !== 0) {
    ...
    for (let i = limit; i >= 0; i--) {
      const $name = $("<div>")
        .addClass("leaderboardList")
        .text(sortedLeaderboard[i][0]);
      const $level = $("<div>")
        .addClass("leaderboardList")
        .text(sortedLeaderboard[i][1]);
      $(".highestLevel").after($name);
      $name.after($level);
    }
  } else {
    $(".highestLevel").after(
      $("<p>").text("No one has played yet.").addClass("emptyLeaderboard")
    );
  }
});
...
const main = () => {
  window.onkeydown = (event) => {
    //finds out which arrow key is pressed and updates playerPosition, precedingPoint, and followingPoint
    if (app.original) {
      app.currentMap = originalMapProcessing();
    }
    ...
    if (
      ...
      restrictedRow1 >= 0 &&
      restrictedColumn1 >= 0 &&
      restrictedRow1 < app.currentMap.length
    ) {
      //cannot have - numbers and also over the grid numbers
      ...
      if (
        ...
        //check restriction: whether the player has a wall infront of them or not
...
const updateSymbols = () => {
  for (let i = 2; i >= 0; i--) {
    ...
    if (row >= 0 && column >= 0) {
      ...
      if (i === 0) {
        //preceding points can only change to floor or goal
        if (point === "@") {
          //player
        ...
        //player points can only change to player or player on goal
        if (point === " " || point === "$") {
          //floor or box
        ...
        //following points only need to change to box or box on goal, all depending on prevpoint
        if (point === " ") {
